...
def test_double_resupply_in_place_replacement() -> None:
    ...
    assert (
        event_entry.double_resup_desc == 'Double-resupply by AmmoX and MedicY'
    )
...
def test_generator_missile_lock_and_followup() -> None:
    ...
    assert len(gen.player_event_log) == 2
    assert (
        gen.player_event_log[1].follow_up_desc == 'Missile from Player2 MISSES'
    )
    ...
    assert any(
        s is not None and s.text == 'Missile from Player2 MISSES'
        for s in slots_8000
    )
    ...
    config = {
        ...
        'elements': {
            ...
            'el_game': {
                ...
                'end_time_offset': 'beginning of game',
                'visible_start_ms': 1000,
                'visible_end_ms': 20000,
            },
            'el_end': {
...
def test_generator_end_time_offset() -> None:
    from datetime import datetime
    from lfdata.model import LFGame
    from lfdata.video import VisualElementGenerator
    ...
    # 1. default offset is beginning of game (offset = 5000)
    # visible_end_ms = 20000 + 5000 = 25000
    el_default = gen._create_ui_element('el_default', element_type='text')
    assert el_default.visible_end_ms == 25000

    # 2. beginning of video offset (offset = 0)
    # visible_end_ms = 20000 + 0 = 20000
    el_video = gen._create_ui_element('el_video', element_type='text')
    assert el_video.visible_end_ms == 20000

    # 3. beginning of game offset (offset = 5000)
    # visible_end_ms = 20000 + 5000 = 25000
    el_game = gen._create_ui_element('el_game', element_type='text')
    assert el_game.visible_end_ms == 25000

    # 4. end of game offset (offset = 5000 + 100000 = 105000)
    # visible_end_ms = -5000 + 105000 = 100000
    el_end = gen._create_ui_element('el_end', element_type='text')
    assert el_end.visible_end_ms == 100000

    # 5. default (beginning of game) offset with 0 -> video_end_ms
    # 100000 duration + 10000 extra + 5000 pregame = 115000 ms
    el_default_zero = gen._create_ui_element(
        'el_default_zero', element_type='text'
    )
    assert el_default_zero.visible_end_ms == 115000

    # 6. beginning of video offset with 0 -> 0 + 0 = 0
    el_video_zero = gen._create_ui_element('el_video_zero', element_type='text')
    assert el_video_zero.visible_end_ms == 0

    # 7. end of game offset with 0 -> 0 + 105000 = 105000
    el_end_zero = gen._create_ui_element('el_end_zero', element_type='text')
    assert el_end_zero.visible_end_ms == 105000
...
def test_generator_player_events_max_lines_and_fade_duration() -> None:
    ...
    player_evs = [
        el
        for el in elements
        if el.element_type == 'text' and el.text == 'Zap Player2'
    ]
